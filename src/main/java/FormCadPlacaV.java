...
public class FormCadPlacaV extends javax.swing.JDialog {
    ...
    private BDGerProd gerPlacaV = BDGerProd.gerGerPlacaV();
                    ...
                    .addGroup(layout.createSequentialGroup()
                        ...
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addGroup(layout.createSequentialGroup()
                                ...
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(cxGarantia, javax.swing.GroupLayout.DEFAULT_SIZE, 89, Short.MAX_VALUE))
                            .addGroup(layout.createSequentialGroup()
    ...
    public void cadastrarPlacaV(){
        ...
        placaV = gerPlacaV.cadPlacaV(placaV);
        limpa();
    }

    public void alterarPlacaV() {
    PlacaDeVideo placaVBusca = new PlacaDeVideo(); 
    
    try { 
        placaVBusca.setMarca(cxMarca.getText());
        placaVBusca.setModelo(cxModelo.getText());

            int confirmacao = JOptionPane.showConfirmDialog(
                null, 
                "Tem certeza que deseja alterar esta Placa De Video?",
        ...
        placaV = gerPlacaV.alteraPlacaV(placaVBusca);
        
        if(placaV != null){
            JOptionPane.showMessageDialog(
                    null,
                    "Preço e Garantia da Placa De Video atualizados com sucesso!",
                    "Atualização de Placa De Video",
                    1
            );
            limpa();
        }
            ...
            JOptionPane.showMessageDialog(
                    null,
                    "Não existe Placa De Video com esta Marca e Modelo.",
    ...
    public void consultarPlacaV(){
       ...
       try{
        placaV.setMarca(cxMarca.getText());
        placaV.setModelo(cxModelo.getText());
        placaV = gerPlacaV.consPlacaV(placaV);
        
        if(placaV != null){
            cxModelo.setText(placaV.getModelo());
            cxMarca.setText(placaV.getMarca());
            cxAno.setText(Integer.toString((int) placaV.getAno()));
            cxPreco.setText(Float.toString((float) placaV.getPreco()));
            cxMemoria.setText(Integer.toString((int)placaV.getMemoria()));
            cxVeloVram.setText(Float.toString((float)placaV.getVeloVram()));
            cxTipoRam.setText(placaV.getTipoMemoria());
            cxNumPci.setText(Float.toString((float)placaV.getNumPci()));
            cxSaidaVideo.setText(placaV.getSaidaVideo());
            cxComprimento.setText(Float.toString((float)placaV.getDime().getComprimento()));
            cxLargura.setText(Float.toString((float)placaV.getDime().getLargura()));
            cxEspessura.setText(Float.toString((float)placaV.getDime().getEspessura()));
            cxGarantia.setText(placaV.getDataGarantia());
            JOptionPane.showMessageDialog(
                    null,
                    "Verifique As Informaçoes",
                    "Consulta De Placa De Video",
                    QUESTION_MESSAGE
            );
            limpa();
        }
            ...
            JOptionPane.showMessageDialog(
                    null,
                "Não há uma Placa De Video registrado com essa Marca e Modelo",
                "Placa De Video inexistente",
                0
            );        
            limpa();
    ...
    int confirmacao = JOptionPane.showConfirmDialog(
            null, 
            "Tem certeza que deseja remover este processador?", // Mensagem genérica
            "Confirmação de Remoção", 
            JOptionPane.YES_NO_OPTION
        );
    ...
    PlacaDeVideo placaVParaRemover = new PlacaDeVideo();
        try {
            placaVParaRemover.setMarca(marca);
        } 
        ...
        try {
            placaVParaRemover.setModelo(modelo);
        }
        catch (ProcModInvException pmie) {
            JOptionPane.showMessageDialog(
                    null,
    ...
    PlacaDeVideo placaVRemovido = gerPlacaV.removePlacaVPorMarcaModelo(placaVParaRemover);

    if (placaVRemovido != null) {
        JOptionPane.showMessageDialog(
                null,
